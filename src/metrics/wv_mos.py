...
class Wav2Vec2MOS(nn.Module):
    ...
    def calculate_one(self, audio: Tensor):
        audio_tensor = torchaudio.functional.resample(audio.squeeze(0), 22050, 16000)
        x = self.processor(audio_tensor, return_tensors="pt", padding=True, sampling_rate=16000).input_values
        with torch.no_grad():
            if self.cuda_flag:
                x = x.cuda()
            res = self.forward(x).mean()
